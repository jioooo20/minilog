...
<template>
  ...
  <div class="space-y-4">
    ...
    <div class="flex justify-end gap-2">
      <button @click="markAllAsRead" class="rounded bg-rose-600 px-3 py-1 text-xs font-semibold text-white">Mark All As Read</button>
    </div>
        ...
        <li v-for="n in notifications" :key="n.notif_id" class="rounded-lg border border-slate-200 bg-white p-3">
          <div class="flex items-start justify-between gap-3">
            <div class="flex-1">
              ...
              <div v-if="n.incident?.verification_notes || n.incident?.verified_at || n.incident?.resolved_at" class="mt-2 space-y-1 rounded-lg bg-slate-50 p-3 text-xs text-slate-600">
                <div v-if="n.incident?.verified_at"><span class="font-semibold text-slate-700">Verification Date:</span> {{ formatDate(n.incident.verified_at) }}</div>
                <div v-if="n.incident?.resolved_at"><span class="font-semibold text-slate-700">Repair Completed:</span> {{ formatDate(n.incident.resolved_at) }}</div>
                <div v-if="n.incident?.verification_notes"><span class="font-semibold text-slate-700">Verification Notes:</span> {{ n.incident.verification_notes }}</div>
              </div>
              <div class="mt-1 text-xs text-slate-500">{{ n.created_at }}</div>
              <div v-if="n.incident" class="mt-2 text-xs"><Link :href="route('incidents.show', n.incident.incident_id)" class="text-indigo-600">View Incident: {{ n.incident.title }}</Link></div>
            </div>

            <div class="flex items-center gap-2">
              <button v-if="!n.is_read" @click="markAsRead(n)" class="rounded bg-emerald-600 px-3 py-1 text-xs font-semibold text-white">Mark as Read</button>
            </div>
          </div>
        </li>
      </ul>
